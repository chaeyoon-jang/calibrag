...
class ClassificationTuner(Trainer):
    ...
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        soft_values = class_labels.float() / 10.0
        soft_labels = torch.stack([soft_values, 1 - soft_values], dim=1)
    ...
    def evaluate(self, eval_dataset=None, metric_key_prefix="eval", **_):
        ...
        soft_values = all_labels.float() / 10.0 
        soft_labels = torch.stack([soft_values, 1 - soft_values], dim=1) 
        brier_score = torch.mean(torch.sum((all_probs - soft_labels) ** 2, dim=1)).item()
        ...
        def compute_ece(probs, soft_targets, n_bins=15):
            confidences, predictions = torch.max(probs, dim=1)
            predicted_confidences = confidences
            predicted_correctness = soft_targets.gather(1, predictions.unsqueeze(1)).squeeze(1)  
            ...
            for i in range(n_bins):
                ...
                if prop_in_bin > 0:
                    avg_confidence_in_bin = predicted_confidences[in_bin].mean()
                    avg_correctness_in_bin = predicted_correctness[in_bin].mean()
                    ece += torch.abs(avg_confidence_in_bin - avg_correctness_in_bin) * prop_in_bin

            return ece.item()

        ece_score = compute_ece(all_probs, soft_labels)

        metrics = {
            f"{metric_key_prefix}_N": all_labels.size(0),
            f"{metric_key_prefix}_acc": (all_logits.argmax(dim=-1) == torch.argmax(soft_labels, dim=1))
            .float()
            .mean()
            .item(),
            f"{metric_key_prefix}_loss": - (soft_labels * log_probs).sum(dim=1).mean().item(),
...
def main(
    ...
    with accelerator.main_process_first():        
        if prev:
            print('Train with previous data')
            train_data = pd.read_csv("/mnt/home/chaeyun-jang/calibrag/code/logs/golden/test_train.csv")
            train_data = train_data.sample(frac=1, random_state=seed).reset_index(drop=True)
            valid_data = pd.read_csv("/mnt/home/chaeyun-jang/calibrag/code/logs/golden/test_valid.csv")
        else:
